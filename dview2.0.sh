...
proc SetFile {} {
  if {$domain == 0} {
 set fname [tk_getSaveFile -initialdir [file dirname $outputFile] -filetypes {{"Xnmr data" {.dat}} {All {*}}}]
  } else {
 set fname [tk_getSaveFile -initialdir [file dirname $outputFile] -filetypes {{"Xnmr data" {.ft}} {All {*}}}]
  }
}
proc SaveConfig { } {
